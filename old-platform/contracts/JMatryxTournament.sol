    ...
    function() public {
        assembly {
            ...
            function whileEnterable(offset) {
                let s := getState(offset)
                require(or(lt(s, 2), eq(s, 2))) // <= LibEnum.TournamentState.Open
            }
            ...
            function allocateMoreToRound(offset) {
                ...
                mstore(ptr, mul(0x5fd45f0b, offset))    // allocateMoreToRound(LibTournamentStateManagement.StateData storage,uint256,address)
