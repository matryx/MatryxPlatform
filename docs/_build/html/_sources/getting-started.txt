...
Approving Entry Fee Withdrawals
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

You'll now be able to enter tournaments, granted you've paid the entry fee. Let's try that, shall we? In order to enter a tournament, you'll first need to tell MatryxToken that you approve of the tournament collecting the fee designated by the tournament's creator. This is done by calling

.. code-block:: Solidity

	token.approve(tournament.address, tournament.entryFee());

where 'token' is a web3 Contract representing MatryxToken. This fee is incurred solely to prevent malicious actors from attemping a cross-tournament Sybil attack and will be returned to you at the end of each tournament you enter.
...
.. code-block:: Solidity
	...
	platform.enterTournament(tournament.address, {gas: platform.enterTournament.estimateGas(tournament.address)});
