...
const logState = async (tournament, accountNumber) => {
  ...
  const rstate = await round.getState().then(Number)
  //console.log("State of Round: " + roundState[tstate])
  console.log(chalk`Tournament state: {magenta ${tournamentState[tstate]}}`)
  console.log(chalk`Round state: {magenta ${roundState[rstate]}}`)
...
module.exports = async exit => {
  try {
    ...
    // await sleep(5000)
    let t3 = await logState(tournament3, 1)
    ...
    let t4 = await logState(tournament4, 1)
    ...
    await sleep(3000)
    let t5 = await logState(tournament5, 1)

    let round_state6 = {
        start: Math.floor(Date.now() / 1000),
        end: Math.floor(Date.now() / 1000) + 4,
    ...
    await sleep(5000)
    let t6 = await logState(tournament6, 1)
...
/*
  ...
  const tournament = await createTournament(web3.toWei(10), roundData_s0, tournamentCreator)
  ...
  await selectWinnersWhenInReview(tournament, tournamentCreator, submissions, submissions.map(s => 1), roundData_s1, 1)
  timeouts.forEach(t => clearTimeout(t))
  await sleep(1000)
