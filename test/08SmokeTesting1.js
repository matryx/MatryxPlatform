...
contract('Tournament and Round: NotYetOpen', function(accounts) {
    ...
    it("Able to create a tournament with a round that's NotYetOpen", async function () {
        ...
        var startTime = Math.floor((new Date() / 1000 + 1000000));
        ...
        assert.isTrue(tournamentState == 0 && roundState == 0, "Tournament and round were not NotYetOpen");
    });

    it ("Tournament owner able to edit tournament details while the tournament is NotYetOpen", async function() {
    	function stringToBytes32(text, requiredLength) {var data = ethers.utils.toUtf8Bytes(text); var l = data.length; var pad_length = 64 - (l*2 % 64); data = ethers.utils.hexlify(data);data = data + "0".repeat(pad_length);data = data.substring(2); data = data.match(/.{1,64}/g);data = data.map(v => "0x" + v); while(data.length < requiredLength) { data.push("0x0"); }return data;}
    ...
    it ("Round owner able to edit round details while the orund is NotYetOpen", async function() {
    	//TODO
    });

    it("Unable to make submisisons while the round is still NotYetOpen", async function () {
        await token.approve(MatryxPlatform.address, "100000000000000000000")

        console.log("New Token Allowance Approved");
        console.log("Attempting to enter tournament...");

        roundState = await r.getState();
        console.log("round state: " + roundState);
