...
contract TestLibPlatform {
    ...
    function allowance(address from, address to) public view returns (uint256) { return allowed; }
    function transferFrom(address from, address to, uint256 value) public view returns (bool) { return true; }
...
// mapping(address=>bool) blacklist;                                        // user blacklist
    ...
    function testGetTournaments() public
    {
        ...
        Assert.equal(tournaments[0], address(this), "First tournament incorrect.");
        Assert.equal(tournaments[1], msg.sender, "Second tournament incorrect.");
    ...
    function testGetSubmission() public
    {
        ...
        bytes32 submissionHash = keccak256(abi.encodePacked(msg.sig));
        data.submissions[submissionHash] = submission;

        LibTournament.SubmissionData memory returnedSubmission = LibPlatform.getSubmission(address(this), address(this), data, submissionHash);
    ...
    function testCreateTournament() public
    {
        ...
        address tournament = LibPlatform.createTournament(address(this), address(this), info, data, tDetails, rDetails);
        Assert.equal(data.allTournaments.length, 1, "There should be exactly one tournament.");
        Assert.equal(data.tournaments[tournament].info.version, 1, "Tournament version should be 1.");
        Assert.equal(data.tournaments[tournament].info.owner, address(this), "Owner of tournament should be this");
        Assert.equal(data.tournaments[tournament].details.bounty, 50, "Tournament bounty should be 50.");
        Assert.equal(data.totalBalance, 50, "Total balance of platform should be 50.");
        Assert.equal(data.tournamentBalance[tournament], 50, "Tournament balance should be 50.");
        Assert.equal(data.tournaments[tournament].rounds[0].details.bounty, 50, "Round bounty should be 50.");
        Assert.equal(data.tournaments[tournament].rounds[0].details.duration, 2, "Round duration should be 2.");

        delete data.whitelist[address(this)];
        delete info.token;
        delete info.system;

        delete data.allTournaments;
        delete data.totalBalance;
        delete data.tournamentBalance[tournament];
        delete data.tournaments[tournament];
    }
}